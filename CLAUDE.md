...
Node.js + Apollo Server + MongoDB GraphQL service that mirrors the
`Lakbay.Contracts` schema exactly. Its entire reason to exist is so
`Lakbay.Web` can be built, tested, and demoed before `Lakbay.Cms`/
...
Not yet proven — Phase 0 is not complete. Once the Apollo Server +
MongoDB (Docker Compose) baseline boots locally, the exact commands go in
